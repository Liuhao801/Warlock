...
class Pool:  #匹配池
    ...
    def match_success(self,ps):
        ...
        players=[]
        for p in ps:
            async_to_sync(channel_layer.group_add)(room_name,p.channel_name)  #将匹配的玩家加到同一个组里
            players.append({
                'uuid':p.uuid,
                'username':p.username,
                'photo':p.photo,
        ...
        while len(self.players)>=3:
            ...
            for i in range(len(self.players)-2):
                ...
                if self.check_match(a,b) and self.check_match(b,c) and self.check_match(a,c):
                    ...
                    self.players=self.players[:i]+self.players[i+3:]  #删除a,b,c
                    flag=True
                    break
            if not flag:
                break
...
def get_player_from_queue():  #返回queue中一名玩家,队列为空返回None
    try:
